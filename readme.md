...
### DATA PREPARATION
The obvious way to sort the raw data is by following the UFC guidelines. That is; sort the data by gender, and then sort each gender into their current weight division. Unfortunately, the raw data does not contain weight-division and gender attributes. Sorting the raw data by weight-division was trivial in the other hand sorting by gender required a gender-classifyer.

### BUILDING THE GENDER CLASSIFYER
I wrote a brute-force search algorithm using Python that [predicts fighters' gender](https://github.com/estgarci/UFC-Data-Analysis/blob/main/name_sex_classifier/sex_classifier.py) given a fighter’s name. The algorithm uses historical names from the U.S national database(www.datagov.org) to predict the likelyhood that a name is traditionally male or female. 
